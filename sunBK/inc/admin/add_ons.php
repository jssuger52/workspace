...
<div class="wrap" id="dp-add-ons">
	...
	<a href="http://digipress.digi-state.com/add-ons/?utm_source=plugin-addons-page&utm_medium=plugin" class="button-primary" title="<?php _e( 'Check other add-ons', 'DigiPress' ); ?>" target="_blank"><?php _e( 'Check other add-ons', 'DigiPress' ); ?></a></p>
<?php 
	// Licenses
...
function dp_add_ons_get_feed() {
	if ( false === ( $cache = get_transient( 'digipress_add_ons_feed' ) ) ) {
		$feed = wp_remote_get( 'https://digipress.digi-state.com/?feed=addons', array( 'sslverify' => false ) );
		if ( ! is_wp_error( $feed ) ) {

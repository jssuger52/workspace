...
if ( function_exists('register_sidebar') ) {
	register_sidebar(array(
			...
			'after_widget'	=> '</div>',
			'before_title'	=> '<h1 class="dp-widget-title"><span>',
			'after_title'	=> '</span></h1>'));
	
	// Top Toggle header
	register_sidebar(array(
			'name'		=> __('Slide toggle area in header', 'DigiPress'),
			'id'		=> 'widget-header-toggle-content',
			'description'	=> __('Slide toggle widget area in site header.', 'DigiPress'),
			'before_widget'	=> '<div class="dp-widget-hd-toggle'.$slider_css.'">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h3 class="toggle-title"><span>',
			'after_title'	=> '</span></h3>'));

	// Top Header right
	register_sidebar(array(
			'name'		=> __('Site header area', 'DigiPress'),
			'id'		=> 'widget-site-header',
			'description'	=> __('Widget area in site header.', 'DigiPress'),
			'before_widget'	=> '',
			'after_widget'	=> '',
...
if ($options_visual['footer_col_number'] == '4') {
	...
	register_sidebar(array(
			...
			'description'	=> __('Between under header area and main content.', 'DigiPress'),
			'before_widget'	=> '<div  id="%1$s" class="dp-widget-content clearfix'.$slider_css.'">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h3 class="inside-title"><span>',
			'after_title'	=> '</span></h3>'));
	...
	register_sidebar(array(
			...
			'before_widget'	=> '<div id="%1$s" class="dp-widget-content clearfix %2$s'.$slider_css.'">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h3 class="inside-title"><span>',
			'after_title'	=> '</span></h3>'));
	...
	register_sidebar(array(
			...
			'id'		=> 'widget-bottom-container-mobile',
...
function DP_GET_POSTS_BY_QUERY($args = array(
	...
	$thumbnail 		= isset($thumbnail) ? true : false;
	$views 			= isset($views) ? true : false;
	$comment 		= isset($comment) ? true : false;
	$order_by		= !empty($order_by) ? $order_by : 'post_date';
	$order			= !empty($order) ? $order : 'DESC';
	$hatebuNumber 	= isset($hatebuNumber) ? true : false;
	$tweetsNumber 	= isset($tweetsNumber) ? true : false;
	$likesNumber 	= isset($likesNumber) ? true : false;
	...
	// *********************
	if (!empty($more_text)) {
		if ( ($type == 'recent' || $type == 'custom')  && $order_by != 'rand') {
			if ($post_type == 'post') {
				if ($cat_id) {
			...
			if ( $more_url ) {
				$more_url = '<a href="'.$more_url.'" class="more-entry-link"><span>'.$more_text.'</span></a>';
			}
		}
	}
	...
	if ($thumbnail) :
		...
		echo '<ul class="recent_entries thumb'.$mobile_class.'">';
		...
		foreach( $posts as $post ) : setup_postdata($post);
			...
			if ( $voted_count ) {
				if (!empty($voted_icon)) {
					$voted_icon_class = ' class="'.$voted_icon.'"';
				}
			...
			if ( $views && function_exists('dp_get_post_views') ) {
				$views_code = '<span class="icon-eye ft10px">'.dp_get_post_views(get_the_ID(), $meta_key).'</span>';
			}
			if ($post_type === 'post') {
				// ************* SNS sahre number *****************
				// hatebu
			...
			echo '<li id="'.$unique_id.'" class="clearfix"><div class="widget-post-thumb">';
			echo show_post_thumbnail($width, $height);
			echo $ranking_code.'</div><div class="excerpt_div clearfix"><div class="excerpt_title_div">';

			// Publish date
			if ( (bool)$pub_date && !get_post_meta(get_the_ID(), 'dp_hide_date', true) ) {
				$date_code = '<time datetime="' . get_the_date('c') . '" pubdate="pubdate" class="widget-time">' . get_the_date() . '</time>';
			}
			echo $date_code;
...
?>
<h4 class="excerpt_title_wid"><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php echo $post_title; ?></a></h4> <span class="icon-comment ft10px"><?php comments_popup_link(' 0 ', ' 1 ', ' % '); ?></span> <?php echo $views_code . $sns_share_code; ?>
		<?php
			} else {
				echo '<h4 class="excerpt_title_wid"><a href="'.get_permalink().'" title="'. get_the_title() .'">' . $post_title .'</a></h4> ' . $views_code . $sns_share_code;
			}
			echo '</div></div>'.$js_code.'</li>';
		endforeach;
		// Reset Query
		wp_reset_postdata();
		...
		foreach( $posts as $post ) : setup_postdata($post);
			...
			// Display
			// ***********************
			// Has SNS share number
			if ($unique_id) {
				$listStartCode = '<li id="'.$unique_id.'" class="clearfix"><div class="excerpt_div clearfix"><div class="excerpt_title_div">';
				$js_code 		= '<script>j$(function() {get_sns_share_count("'.get_permalink().'", "'.$unique_id.'");});</script>';
			} else {
				$listStartCode = '<li class="clearfix"><div class="excerpt_div clearfix"><div class="excerpt_title_div">';
				$js_code 		= '';
			}
			...
			if (($post_type === 'post') && $comment && comments_open()) : ?>
<?php echo $date_code; ?><h4 class="excerpt_title_wid"><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php echo $post_title; ?></a></h4> <span class="icon-comment ft10px"><?php comments_popup_link(' 0 ', ' 1 ', ' % '); ?></span> <?php echo $views_code . $sns_share_code; ?>
<?php 
			else :
?>
<?php echo $date_code; ?><h4 class="excerpt_title_wid"><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php echo $post_title; ?></a></h4> <?php echo $views_code . $sns_share_code;
			endif;

			echo '</div>'.$ranking_code.'</div>'.$js_code.'</li>';
		endforeach;
		// Reset Query
		wp_reset_postdata();
	endif;
...
class DP_RecentPosts_Widget extends WP_Widget {
	...
	function form($instance) {
		...
		echo '<p><label for="'.$more_text_id.'">'.__('More label','DigiPress').':</label><br /><input type="text" name="'.$more_text_name.'" id="'.$more_text_id.'" value="'.$more_text.'" style="width:100%;" /></p>';
...
class DP_Most_Viewed_Posts_Widget extends WP_Widget {
	function DP_Most_Viewed_Posts_Widget() {
		...
		$control_opts = array('width' => 200, 'height' => 300);
		$this->WP_Widget('DPMostViewedPostsWidget', __('Most Viewed Posts Widget', 'DigiPress'), $widget_opts, $control_opts);
	}
	...
	function form($instance) {
		...
		$likesNumber = $instance['likesNumber'];
		$meta_key 	= $instance['meta_key'];
		$reset_weekday 	= $instance['reset_weekday'];
...
class DP_MostCommentedPosts_Widget extends WP_Widget {
	function DP_MostCommentedPosts_Widget() {
		...
		$control_opts = array('width' => 200, 'height' => 300);
		$this->WP_Widget('DPMostCommentedPostsWidget', __('Most Commented Posts Widget', 'DigiPress'), $widget_opts, $control_opts);
	}
	...
	function widget($args, $instance) {
		...
		if ($instance['rss']) {
			if ($instance['to_feedly']) {
				$rss = '<li><a href="//cloud.feedly.com/#subscription%2Ffeed%2F'.urlencode(get_feed_link()).'" target="blank" title="Follow on feedly" class="icon-feedly"><span>Follow on feedly</span></a></li>';
			} else {
				$rss = '<li><a href="'
...
class DP_Facebook_Like_Box_Widget extends WP_Widget {
	...
	function form($instance) {
		...
		$border_name		= $this->get_field_name('border');
		$border_id			= $this->get_field_id('border');
		...
		$strTag = '<p><label for="'.$app_id_id.'">'.__('App ID', 'DigiPress').'(*):</label><br />' . 
			 ...
			 <label for="'.$show_header_id.'">'.__('Show Header', 'DigiPress').'</label><br /><span style="font-size:11px;">' . __('*Note : Show the "Find us on Facebook" bar at top. Only shown when either stream or faces are present.', 'DigiPress') . '</span></p>' . 
			 	  '<p><input name="'.$border_name.'" id="'.$border_id.'" type="checkbox" value="true" '.$border_check.' />
			 	  <label for="'.$border_id.'">'.__('Show Border', 'DigiPress').'</label></p>';
	...
	function widget($args, $instance) {
		...
		if ($instance['app_id'] != '' && $instance['url'] != '') {
			...
			$tagBorderColor	= ($instance['border']) ? ' data-show-border="true"' : ' data-show-border="false"';
